...
def remove_licenses(package):
	if "licenses" in package:
		del package["licenses"]
	return package
...
if __name__ == "__main__":
	...
	pkg = [remove_licenses(p) for p in packages]
	pkg = [remove_versions(p) for p in packages]
	with open("pypi_top_n.json", "w", encoding = "utf-8") as f:
		json.dump(pkg, f, indent = 2, ensure_ascii = False)
	with open("pypi_all_responses.json", "w", encoding = "utf-8") as f:
		json.dump(responses, f, indent = 2, ensure_ascii = False)
	print("Complete...Saved",len(packages),"packages in pypi_top_n.json")
